...
class StreamManager(object):
    ...
    def _handler(self, heap):
        ...
        key = (int(items["feng_idx"].value), int(items["chan_idx"].value))
    ...
    def _finish(self):
        for key in sorted(self._counts.keys()):
            ...
            print("feng_idx={}, chan_idx={}, power={}".format(
                key[0], key[1], total_power.sum()/float(count)))
            np.save("{}_{}.npy".format(key[0], key[1]), total_power)
...
def main(mcast_groups, interface, nheaps):
    ...
    for group in mcast_groups:
        stream.add_udp_reader(group, 7148, interface_address=interface)
