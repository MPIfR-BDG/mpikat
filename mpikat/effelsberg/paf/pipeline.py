...
# Epoch of BMF timing system, it updates every 0.5 year, [UTC datetime, EPOCH in BMF packet header]
EPOCHS = [
...
class Pipeline(object):
    ...
    def _acquire_beam_index(self, ip, port, ndf_check_chk):
        ...
        socket.setdefaulttimeout(self._prd)  # Force to timeout after one data frame period
    ...
    def _synced_refinfo(self, utc_start_capture, ip, port):
        ...
        idf_ref = int((utc_start_capture.unix - epoch_ref * 86400.0 - sec_ref)/self._df_res)
...
class Search(Pipeline):    
    ...
    def configure(self, utc_start_capture, freq, ip, pipeline_config):
        ...
        for i in range(self._nbeam):
            if EXECUTE:
                ...
                socket_address    = "{}/beam{:02}/capture.socket".format(DATA_ROOT, beam_index)
            ...
            self._filterbank_delete_buffer_commands.append("dada_db -d -k {:}".format(self._rbuf_filterbank_key[i]))

            # command to delete baseband ring buffer
            self._baseband_delete_buffer_commands.append("dada_db -d -k {:}".format(self._rbuf_baseband_key[i]))
            ...
            if self._zap_chans:
                ...
                for zap_chan in self._zap_chans:
                    zap += " -zap_chans {} {}".format(self._zap_chan[0], self._zap_chan[1])
                command += zap
                if self._dm:
        ...
        for command in self._capture_commands:
            ...
            execution_instance.stdout_callbacks.add(self._decode_capture_stdout)
            execution_instance.error_callbacks.add(self._handle_execution_error)
    ...
    def start(self, utc_start_process, source_name, ra, dec):
        ...
        for command in self._baseband2filterbank_commands:
            baseband2filterbank_execution_instance = ExecuteCommand(command, resident = True)
            baseband2filterbank_execution_instance.stdout_callbacks.add(self._decode_baseband2filterbank_stdout)
            baseband2filterbank_execution_instance.error_callbacks.add(self._handle_execution_error)
            self._baseband2filterbank_execution_instances.append(baseband2filterbank_execution_instance)

        if HEIMDALL: # run heimdall if required
            self._heimdall_execution_instances = []
            for command in self._heimdall_commands:
                heimdall_execution_instance = ExecuteCommand(command, resident = True)
                heimdall_execution_instance.error_callbacks.add(self._handle_execution_error)
                self._heimdall_execution_instances.append(heimdall_execution_instance)
        if DBDISK:   # Run dbdisk if required
            self._dbdisk_execution_instances = []
            for command in self._dbdisk_commands:
                dbdisk_execution_instance = ExecuteCommand(command, resident = True)
                dbdisk_execution_instance.error_callbacks.add(self._handle_execution_error)
                self._dbdisk_execution_instances.append(dbdisk_execution_instance)
    ...
    def deconfigure(self):
        ...
        if self.state == "ready": # Normal deconfigure
            ...
            if EXECUTE:
                index = 0
                for control_socket in self._control_socket:
                    self._capture_control(control_socket, "END-OF-CAPTURE", self._socket_address[index])
                    index += 1
            for execution_instance in self._capture_execution_instances:
                execution_instance.finish()                    
            print self.state, "HERE\n"
...
class Search2Beams(Search):
    ...
    def configure(self, utc_start_capture, freq, ip):
        super(Search2Beams, self).configure(utc_start_capture, freq, ip, SEARCH_CONFIG_2BEAMS) 
...
class Search1Beam(Search):
    ...
    def configure(self, utc_start_capture, freq, ip):
        super(Search1Beam, self).configure(utc_start_capture, freq, ip, SEARCH_CONFIG_1BEAM) 
