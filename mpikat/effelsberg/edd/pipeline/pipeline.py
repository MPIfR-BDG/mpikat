...
class Mkrecv2Db2Dspsr(object):
    ...
    def configure(self):
        ...
        ###################################
        self.cmd = "dada_db -k {key} {args}".format(**
                                               self._config["dada_db_params"])
        log.debug("Running command: {0}".format(self.cmd))
        #args = shlex.split(cmd)
        self._create_ring_buffer = safe_popen(self.cmd, stdout=PIPE)
    ...
    def start(self):
        ...
        self.cmd = "mkdir -p {}".format(out_path)
        log.debug(self.cmd)
        log.debug(os.getcwd())
        #args = shlex.split(cmd)
        process = safe_popen(self.cmd, stdout=PIPE)
        process.wait()
        os.chdir(out_path)
    ...
    #log.debug("Header file contains:\n{0}".format(header_string))
        ...
        # Start up DSPSR
        ###################
        self.cmd = "dspsr {args} -N {source_name} {keyfile}".format(
            args=self._config["dspsr_params"]["args"],
            source_name=source_name,
            keyfile=dada_key_file.name)
        #args = shlex.split(cmd)
        log.debug("Running command: {0}".format(self.cmd))
        self._dspsr = safe_popen(self.cmd, stdout=PIPE)
        ...
        ###################
        self.cmd = "dada_junkdb -k {0} -b 320000000000 -r 1024 -g {1}".format(
            self._dada_key,
            dada_header_file.name)
    ...
    def deconfigure(self):
        ...
        self.cmd = "dada_db -d -k {0}".format(self._dada_key)
        log.debug("Running command: {0}".format(self.cmd))
        #args = shlex.split(cmd)
        process = safe_popen(self.cmd, stdout=PIPE)
        process.wait()
